...
func _ready():
	update_values("critical_damage_intermediate")

func update_values(skill: String):
	skill_per_value.text = str(skill_data.skill_values.get(skill) * 100) + "%"
	...
	skill_total_increase_value.text = str(total_increase * 100) + "%"
