...
func _ready():
	update_values("starting_mana_intermediate")
...
func update_values(skill: String):
	...
	skill_total_increase_value.text = str(skill_data.skill_values.get(skill) * skill_data.skill_current_upgrades.get(skill))
