...
@onready var lives_bar: ProgressBar = $main_panel/VBoxContainer/skill_tree_buttons/lives_basic/ProgressBar
@onready var research_bar: ProgressBar = $main_panel/VBoxContainer/skill_tree_buttons/research_basic/ProgressBar
@onready var critical_chance_bar: ProgressBar = $main_panel/VBoxContainer/skill_tree_buttons/critical_chance_basic/ProgressBar
@onready var critical_damage_bar: ProgressBar = $main_panel/VBoxContainer/skill_tree_buttons/critical_damage_basic/ProgressBar
@onready var starting_mana_bar: ProgressBar = $main_panel/VBoxContainer/skill_tree_buttons/starting_mana_basic/ProgressBar
@onready var range_bar: ProgressBar = $main_panel/VBoxContainer/skill_tree_buttons/range_basic/ProgressBar

#upgrade unlock progress bar selectors
@onready var critical_chance_unlock_bar:ProgressBar = $main_panel/VBoxContainer/skill_tree_buttons/critical_chance_basic_unlock
@onready var critical_damage_unlock_bar: ProgressBar = $main_panel/VBoxContainer/skill_tree_buttons/critical_damage_basic_unlock
@onready var starting_mana_unlock_bar: ProgressBar = $main_panel/VBoxContainer/skill_tree_buttons/starting_mana_basic_unlock
...
@onready var upgrade_map: Array = [
	...
	[lives_bar, "lives", null, 0, 1],
	[research_bar, "research", "starting_mana", 0, 1],
	[range_bar, "range", null, 1, 1],
...
func handle_button_press(current_upgrade_map: Array):
	var current_cost = skill_data.skill_current_cost.get(current_upgrade_map[1] + "_basic")
	var current_upgrades = skill_data.skill_current_upgrades.get(current_upgrade_map[1] + "_basic")
	var max_upgrades = skill_data.skill_max_upgrades.get(current_upgrade_map[1] + "_basic")
	...
	if(Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT)):
		...
		if(current_upgrade_map[0].value < max_upgrades):
			...
			current_upgrades += 1
			skill_data.skill_current_cost.set(current_upgrade_map[1] + "_basic", current_cost)
			skill_data.skill_current_upgrades.set(current_upgrade_map[1] + "_basic", current_upgrades)
		...
		if(current_upgrade_map[2] != null):
			if(skill_data.skill_current_upgrades.get(current_upgrade_map[2] + "_basic") == null):
				return
			if(skill_data.skill_current_upgrades.get(current_upgrade_map[2] + "_basic") > 0):
				return
...
func handle_unlock_bar_update(bar: ProgressBar, previous_upgrade: String):
	if(skill_data.skill_max_upgrades.get(previous_upgrade + "_basic") == null):
		return
	bar.max_value = skill_data.skill_max_upgrades.get(previous_upgrade + "_basic")
	bar.value = skill_data.skill_current_upgrades.get(previous_upgrade + "_basic")
