...
func update_data():
	var total_damage_value: float = get_tree().get_root().get_node("game").water_research_value
	var total_upgrades_value: float = (total_damage_value - 1) / .10
	var cost_value: int = get_tree().get_root().get_node("game/main_ui/Main-ui/research_tab_buttons/HBoxContainer/VBoxContainer/water_research_button").cost
